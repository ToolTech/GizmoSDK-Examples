...
// File			: irshader.cpp
// Module		: 
// Description	: Test app to show infrared and nvg colorizing with shaders
// Author		: Anders Modén          
...
gzNode *createModel()
{
	gzGeometry *geom=new gzGeometrySphere(1);

	gzArray<gzVec3> &coordinates=geom->getCoordinateArray(FALSE);

	gzArray<gzVec4> &colors=geom->getColorArray();

	gzArray<gzVec3>	ircolors;

	// Create a test ir color as a "heat" sample for each vertex

	gzUInt32 size=coordinates.getSize();

	ircolors.setSize(size);	// Set size of ir data

	colors.setSize(size);	// Set size of normal color data

	for(gzUInt32 i=0;i<size;i++)
	{
		// Set the actual ircolor
		ircolors[i]=gzVec3((coordinates[i].x+1)/2.0,0,0)*sin((coordinates[i].x+1)/2*11.0);

		// Set the actual standard color (The color position of the coordinates)
		colors[i]=gzVec4(coordinates[i].x,coordinates[i].y,coordinates[i].z,1);
	}

	geom->setColorBind(GZ_BIND_PER_VERTEX);

	gzVertexAttributeListTemplate<gzVec3> *vaList=new gzVertexAttributeListTemplate<gzVec3>(ircolors,4);
...
public:
	MyWindow(gzGraphicsFormat *format , gzString filename ):gzWindow(gzString("Gizmo3D : ")+gzTime::now(TRUE).asString(),NULL,format,FALSE)
	{
			...
			gzString vertexProgram			="glsl_irshader.vsh";
			gzString fragmentProgram		="glsl_irshader.fsh";

			gzString alt_vertexProgram		="irshader.vsh";


			// Create a object node from a. This model will contain alternate color info
